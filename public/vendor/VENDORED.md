...
Self-hosted brand fonts (latin subset, woff2), replacing the former Google Fonts
`@import` in `styles.css`. First-party so the strict `_headers` CSP
(`style-src 'self'; font-src 'self'`) allows them and the site honors its
"no external CDNs / no egress" promise (also avoids the GDPR IP-leak of remote
Google Fonts loading).

- `dm-serif-display.woff2` / `dm-serif-display-italic.woff2` — DM Serif Display 400 (OFL)
- `sora.woff2` — Sora variable 300..600 (OFL)
- `jetbrains-mono.woff2` — JetBrains Mono variable 300..500 (OFL)

Source: Google Fonts `css2` API (latin subset). Re-fetch by re-running the URLs in
each `@font-face` against the gstatic CDN if a font is ever re-pinned.
